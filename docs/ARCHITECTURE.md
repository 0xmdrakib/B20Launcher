...
The worker checks armed launches at Base's safe head. An uninitialized native token needs no historical log scan. Once initialized, historical state reads locate its first initialized block with binary search, followed by a single-block router-log request. This accommodates Alchemy Free's ten-block log limit and avoids repeated scans for abandoned drafts. Failed state reads never advance a cursor. Existing committed launches are backfilled in pages. Reconciliation checks the canonical launch receipt and native `isB20Initialized`, then reads current name, symbol, decimals and `contractURI` at one safe block. Changed profiles and raster images are size bounded and retrieved only through fixed IPFS gateways; unsupported external URL schemes retain the last verified snapshot and retry. An unchanged immutable URI reuses its verified content to avoid unnecessary gateway traffic. Credential-bearing RPC URLs are never returned by the public status endpoint.
